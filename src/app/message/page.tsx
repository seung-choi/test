...
const Message = () => {
  ...
  return (
    <>
        ...
        <div className={styles["message-container"]}>
          ...
          <div className={styles["content"]}>
                      ...
                      return (
                        <label key={course.courseId}>
                          <input
                            ...
                            checked={refinedBookingData?.some(
                              (booking) =>
                                booking.courseId === course.courseId &&
                                bookingList.includes(`${booking.bookingId}`),
                            )}
                          />
            ...
            <button
              ...
              onClick={() => setStep(2)}
              disabled={bookingList.length === 0 || refinedBookingData?.length === 0}
            >
              {t("message.next")}
            </button>
          </div>
      ...
      {step === 2 && (
        <SendMessage groupedByCourse={getGroupedByCourse()} onBack={() => setStep(1)} />
      )}
    </>
  );
};
